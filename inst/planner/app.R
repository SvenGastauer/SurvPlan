...
source('plot_trans.R')
allt=NULL
pkgs = c("shiny", "leaflet", "DT", "ggplot2", "sf", "purr", "dplyr", "sp", "tidyr")
lapply(pkgs, require, character.only = TRUE)

ui <- fluidPage(
  titlePanel(title=fluidRow( div(column(width = 6, tags$img(src = "logo.png", height = 135, width = 284)),
                  column(width = 6, h2("Transect Planner")))),
              windowTitle="Thünen Transect Planner"),
  br(),
  ...
  h3('Map'),
  br(),
  leaflet::leafletOutput("survmap", width = "70%", height = 800),
  p(),
  br(),
...
server <- function(input, output, session) {
  ...
  #create polygon map
  output$survmap <- leaflet::renderLeaflet({
